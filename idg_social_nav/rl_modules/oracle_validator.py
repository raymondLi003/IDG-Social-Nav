...
It is given the same observation as the learned validator, which is a
privileged view of the environment and the proposer action,
and it decides whether to override or obey based on the discomfort of the leader's proposal.

The decision is made by comparing the discomfort to a threshold tau,
which can be set in the model configuration.
...
class OracleValidatorRLM(RLModule):
    ...
    def _candidate_discomfort(
        ...
        """
        Returns the discomfort of the cell that would be executed by the given environment action,
        given the egocentric view of the environment.
        The discomfort is defined as the value of the CH_DISCOMFORT channel in the cell
        that would be executed by the given environment action.
        The cell that would be executed is determined by the following rules:
        - If the action is MOVE_FORWARD and there is a pedestrian in the
          cell ahead, return 1.0 (the discomfort of the pedestrian cell).
        - If the action is MOVE_FORWARD and there is no wall in the cell
          ahead, return the discomfort of the cell ahead.
        - If the action is MOVE_FORWARD and there is a wall in the cell
          ahead, return the discomfort of the agent's current cell.
        - If the action is TURN_LEFT or TURN_RIGHT, return the discomfort
          of the agent's current cell.
